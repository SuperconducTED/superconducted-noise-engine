...
A second convention review of PR #68 found no hard-constraint violation and no
wrong number: every figure below and in the evidence README was re-measured
from the committed TSV and reproduced to the printed digits, the survey
regenerated byte-identically, and `ruff` / `ruff format` / `mypy --strict` /
`check_ids` were already clean. What it did find was that **two of the claims
this doc makes were not actually enforced anywhere**, plus four narrower
defects. All are closed here.

**Two claims that were prose rather than test.**

1. *"Every one of the 975 surveyed feature vectors, through
   `ClampingFeatureExtractor(...)`"* — the step-9 section below says that, but
   the committed conformance test clamped with a bare `np.clip` and never
   constructed the wrapper. FR-12's whole reason for existing is that
   `FuzzyNoiseModel._compute_crisp_params` offers no interception point, so a
   test that inlines the clip exercises everything except the mechanism. The
   acceptance loop now runs through the real wrapper, and a new integration
   test injects it into a `FuzzyNoiseModelEnsemble` on an out-of-box snapshot —
   the UC-7 path the ablation actually runs, and the last open item on the
   FR-12 line of the definition of done.
2. *"`tests/...::test_tanh_branch_taken_on_the_real_quantiles_is_pinned` pins
   it"* — it did not. The test recomputed `inside` from the module's own onset
   formula and asserted `tanh_slope_strategy` agreed with it, which is the
   module agreeing with itself: a layout regression that flipped all three
   features to half-reach would have gone green. The nine `x*_j`, the four that
   land inside their box, and the resulting equal-slope branch are now pinned as
   literals, against the step-5b table below.

**Four defects.**

3. **The FR-5 legacy width was applied to the wrong shapes.** `IntervalGaussianMF`
   under `placement="endpoint"` / `"interior"` inherited `GaussianMF`'s pinned
   `0.25·(hi − lo)` instead of section 6.3's `r_j/√(2 ln 2)` — measured 25.0
   against 21.233 at endpoint on a span of 100, a real divergence on an
   untested branch. FR-5 pins that width for `GaussianMF` alone, because the pin
   exists so `first_ensemble_run`'s own numbers reproduce by equality and that
   script builds Gaussians and nothing else; every other shape takes "the
   section 6.3 widths with `r_j` derived from the equal spacing". Split into
   `_bin_cover_sigmas` (all shapes, all placements) and `_gaussian_sigmas` (the
   Gaussian pin), with the legacy IT2 case now covered.
4. **A non-finite feature passed through the clamp uncounted.**
   `(raw < lo) | (raw > hi)` is `False` for NaN and `np.clip` preserves it, so a
   NaN reached the rule base: every firing strength goes NaN, the defuzzifier
   does *not* raise (the sum is NaN, not zero), `KrausChannelProjector` maps NaN
   to 1.0 and installs **maximal** damping, and `is_identity_damping`
   simultaneously reports that vector as the **identity** channel. Wrong in both
   directions at once with every viability check green — precisely the failure
   NFR-5 is written against. `extract` now raises before the counters move.
   `BasicCalibrationVectorizer` cannot produce a NaN, so this is a guard for the
   wrappers that compose with this one, `OffsetFeatureExtractor` (#64) first.
5. **`anchored_rule_base` accepted a zero-length target.** `int(output_dim or 0)`
   turned a `target_fn` returning a `(0,)` vector into an `output_dim=0` rule
   base that built cleanly. It is 1-D and finite, so it passed both other
   guards. Now rejected.
6. **`snapshot_row`'s "never raises" did not cover a malformed document.** It is
   parsed JSON from an external archive and #63 imports the function to build
   the training set, so an `AttributeError` from inside the qubit walk is a
   contract break, not an edge case. Every container is shape-checked now.

**One thing found and deliberately not fixed here.**
`BasicCalibrationVectorizer.extract` raises `AttributeError` on a malformed
document — it assumes `properties.qubits` is a list of lists of dicts. That is
`calibration/features.py`, which Issue #59 consumes and does not edit (NFR-6,
section 1.2), so `snapshot_row` widens its guard at the call site and records
the result as `malformed document: ...`, distinct from the legitimate "no usable
value" rejection. The upstream fix belongs to Baha, alongside #64.

**One API addition.** `grid_partition` gained a keyword-only `tanh_slopes`
override, rejected for every shape but `TanhMF`. Every surveyed feature measures
`equal-slope`, so without it nothing could exercise the half-reach branch on
real data or show what decision 2's fallback bought. The default is unchanged
and still measured per feature; `tanh_slope_strategy` reports what the default
would choose.

Nothing in the layout, the mapping, the anchors, the survey output or any
reported number changed in this round — the TSV still regenerates
byte-identically, and the step-5b, step-9 and clamp-rate figures below are
unmoved. The suite went from 469 tests to 485.

| File | What this round changed |
| --- | --- |
| `src/superconducted/fuzzy/parameterization.py` | `_bin_cover_sigmas` split out so FR-5's legacy pin applies to `GaussianMF` only; `ClampingFeatureExtractor.extract` rejects a non-finite vector before the counters move; `anchored_rule_base` rejects a zero-length target; `grid_partition` gained the `tanh_slopes` override and `TANH_SLOPE_STRATEGIES`. |
| `scripts/feature_distribution.py` | Every container from the parsed document is shape-checked, so `snapshot_row` never raises on a malformed archive file; a malformed document gets its own rejection reason; the deliberate private import of `_coerce_finite_float` is explained. |
| `tests/test_parameterization.py` | Acceptance run goes through the real `ClampingFeatureExtractor`; new ensemble-injection test; clamp rate and the nine `x*_j` pinned as literals; feature vectors built row-aligned; UC-4 smoke check on `compare_mf_placement`; rejection tests for the legacy IT2 width, non-finite features, zero-length targets and misused `tanh_slopes`. |
| `tests/test_feature_distribution.py` | Malformed-document cases for `snapshot_row`'s "never raises" contract. |
| `docs/implementations/2026-09-08-mf-parameterization.md` | This section. |
| `docs/numerical-claims.md` | NC-021 moved to the new suite size. |
...
**As of the second review round, 2026-09-08** (same laptop, still provisional
under NFR-3; the round-1 figures above are left as measured rather than
overwritten):

- Full suite collects and passes **485** tests, up from the 469 of round 1 and
  **+125** against NC-021's 360 at `2d66f7a`. NC-021 is moved to 485 in the
  follow-up commit that cites the sha it was measured at.
- The 13 `test_probe_historical_properties.py` failures of round 1 do **not**
  reproduce here: all 485 pass, because this environment has
  `qiskit-ibm-runtime` installed. That confirms round 1's reading of them as an
  environment gap rather than a regression.
- `ruff check` / `ruff format --check` / `mypy --strict` / `scripts/check_ids.py`
