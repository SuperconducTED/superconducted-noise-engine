...
- Post-merge failure fix: `Commit snapshot` step was using 
`mv` to move a directory onto an existing non-empty 
directory, failing on every run after the first. Replaced 
with `mkdir -p` plus `mv` of the snapshot file(s) only.
