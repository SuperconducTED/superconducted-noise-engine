...
| NC-021 | Full test-suite size | 287 | `python -m pytest tests/ --collect-only -q -o addopts=""`, measured at `04fa8d1` | 2026-09-05 | Was `149` at `981f324`, then `166` at `1873625`, then `225` at `2af5c16`, and `280` at `947fe3d` · the pipeline-health dashboard adds 7 tests: 2 qubit-scope digest checks, 4 metrics/SVG integration and conformance checks, and 1 idempotent backfill integration test · the complete suite also passed at this reference: 287 passed in 27.50s · `tests/test_file_snapshots.py` skips without `git` and `bash`; see Rule 6 · cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at |
