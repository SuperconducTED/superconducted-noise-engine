...
| NC-041 | Feature-distribution quantiles of the calibration archive, in `BasicCalibrationVectorizer` units | `mean_T1` 98.8191 / 131.7984 / 154.3887 µs · `mean_T2` 67.4411 / 97.0365 / 110.5429 µs · `mean_readout_error` 0.0171326 / 0.0205861 / 0.0374223 (p1 / p50 / p99) | `docs/evidence/feature-distribution/2026-09-08-3d1569d.tsv` via `python -m scripts.feature_distribution --repo . --ref 3d1569d18bcc007c35f3f628f79e678e6061bdc3 --out docs/evidence/feature-distribution/2026-09-08-3d1569d.tsv`, over the 975 rows with all three means present | 2026-09-08 | Microseconds and dimensionless, the units `extract` emits with no scaling (Issue #59 NFR-8) — **not** the seconds `scripts/first_ensemble_run.py::FEATURE_SCALES` uses, which is a separate defect · `[p1, p99]` per feature is the domain box `superconducted.fuzzy.parameterization` partitions and `ClampingFeatureExtractor` clamps onto · computed with `numpy.quantile(..., method="linear")`; a different interpolation moves every center · one row per **file**, so these are file-weighted and not equal-mass over distinct device states (NC-025) · the committed TSV is the audit trail for individual cells |
| NC-042 | Median per-qubit spread within a snapshot, by feature | `mean_T1` 45.7756 µs · `mean_T2` 54.9084 µs · `mean_readout_error` 0.0406946 | Same TSV and command as NC-041: the median over the 975 snapshots of each `*_qubit_std` column | 2026-09-08 | **Sample** standard deviation, `numpy.std(v, ddof=1)`, over exactly the per-qubit values `BasicCalibrationVectorizer.extract` averaged; empty and excluded when fewer than two values are usable · `ddof=1` matches `calibration/features.py::per_qubit_spread` (#64), the eventual single home for this statistic; on the committed fixture `ibm_fez_20260513T121322Z_q72_missing_t1t2.json` (row 1 of the TSV) the T1 value is 45.4847 with `ddof=1` against 45.3377 with `ddof=0` · this is what sizes the IT2 footprint, `sigma_high = sqrt(sigma_low² + s_q²)`, per Issue #59 decision 3 |
| NC-043 | Snapshot files in the calibration archive | 975 | `git ls-tree -r --name-only 3d1569d18bcc007c35f3f628f79e678e6061bdc3 -- snapshots/` counting lines ending `.json`, reproduced as the row count of `docs/evidence/feature-distribution/2026-09-08-3d1569d.tsv` | 2026-09-08 | **Files**, never samples and never distinct device states — NC-025 counts the latter (504 at `f0930b9`) · reconciles Issue #59 step 1's two earlier figures rather than contradicting them: the same command gives 920 at `40a1ff4` and 921 at `43dec77`, both re-measured 2026-09-08, so the archive simply grew as the poller ran · spans 2026-05-13 to 2026-09-06, `ibm_fez` only · 0 of the 975 were rejected by `extract` |
| NC-044 | Wall-clock time for a full feature-distribution survey of the archive | 55-64 s | `python -m scripts.feature_distribution --repo . --ref 3d1569d18bcc007c35f3f628f79e678e6061bdc3 --out ...`, `runtime_seconds` from the printed summary, 975 files at `3d1569d`, three uncontended runs (55.08 s, 61.34 s, 63.70 s) | 2026-09-08 | **Provisional, laptop** (Mert's) · stated as a range because a single figure does not survive a re-run: a fourth run that overlapped a full `pytest` on the same machine took 93.39 s, so this measures the laptop's spare I/O as much as the script — `docs/verification/` holds only Burak-desktop records, and under architect decision C2 this PR does not get its own: the figure is superseded by the next batch record `docs/verification/2026-09-XX-phase-3-batch-N-burak-desktop.md` · one `git show` subprocess and one JSON parse per file, about 1.3 MB each, so roughly 1.2 GB of JSON on a warm object cache · `--limit N` exists so smoke runs do not pay it |
