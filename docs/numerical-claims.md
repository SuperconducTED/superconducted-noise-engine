...
| NC-032 | Calibration capture rate outside a known outage, 2026-08-12..14 | ≤ 70.2% (33 of ≥ 47) | Same enumeration as NC-031: 43 documents served inside the window plus 4 held-but-unserved = ≥ 47 proven to exist; `git ls-tree` on `calibration-data` holds 33 | 2026-09-02 | Upper bound, because the sweep itself undercounts (NC-031) · this window was **not** a flagged outage — it is ordinary operation, so the ≤ 19.3% of NC-029 is not the whole story and the shortfall is not confined to scheduler failures · the 14 documents in the `MISSED` rows are absent from every month directory on the branch and were recoverable at measurement time |
