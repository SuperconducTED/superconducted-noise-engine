...
| NC-021 | Full test-suite size | 360 | `python -m pytest tests/ -q` and `python -m pytest tests/ --collect-only -q -o addopts=""`, both run at `5397bb6` on `feature/issue-57-training-target` | 2026-09-08 | Was `149` at `981f324`, then `166` at `1873625`, then `225` at `2af5c16`, then `248`, `266` and `280` across the #55 rounds (see the retired chain in `docs/implementations/2026-09-02-*`) · #69 originally measured `352` at `fd9bb0d`; this review-fix measurement removes the 40 Issue #60 synthetic-trainer tests and moves the remaining Issue #57 tests to `tests/training/`, yielding 356 collected and passed · the 2026-09-08 review follow-ups add 4 more (3 in `tests/test_interfaces.py` for the section 9.2 immutability round trip, the isinstance narrowing rule and the package-docstring count pin, 1 in `tests/training/test_parameters.py` for section 9.1's unshared-membership case), so 356 + 4 = 360 · direct measurement, never a branch-count derivation · resolved to one row per Rule 6 instead of keeping both, which is the defect `scripts/check_ids.py` rule N1 exists to catch · cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at |
