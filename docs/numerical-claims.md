...
| NC-021 | Full test-suite size | 374 | `python -m pytest tests/ --collect-only -q -o addopts=""` (tail line), run at the #71 merge commit on `mert/duplicate-id-ci-check` | 2026-09-06 | Was `149` at `981f324`, then `166` at `1873625`, then `225` at `2af5c16`, then `248`, `266` and `280` across the #55 rounds (see the retired chain in `docs/implementations/2026-09-02-*`) · this merge combines two branches that each measured it separately and neither figure described the merged tree — #69 measured `352` at `fd9bb0d` (`+72` on `280`, of which 40 are the trainer's `tests/test_anfis.py` and `tests/test_train_anfis_script.py`) and #71 measured `302` at `8ace786` (`+22` for `tests/test_check_ids.py`), so the merged tree is `280 + 72 + 22 = 374`, collected rather than derived · resolved to one row per Rule 6 instead of keeping both, which is the defect `scripts/check_ids.py` rule N1 exists to catch · `tests/test_file_snapshots.py` skips without `git` and `bash`, so a machine lacking either collects 374 but passes 365 with 9 skipped · cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at |
