...
| NC-041 | Pipeline-health backfill at the NC-025 reference | 504 distinct states from 894 documents | `scripts/backfill_state_index.py` and `scripts/pipeline_health.py` over `calibration-data` @ `f0930b9`; local run 2026-09-05 | 2026-09-05 | Reproduces NC-025 using the committed qubit-block definition; index contains one row per snapshot and is idempotent on a second run |
