...
| NC-021 | Full test-suite size | 370 | `python -m pytest tests/ -q` and `python -m pytest tests/ --collect-only -q -o addopts=""`, both run at `8176d79` on `mert/payload-digest-parameter-dates` | 2026-09-08 | Was `149` at `981f324`, then `166` at `1873625`, then `225` at `2af5c16`, then `248`, `266` and `280` across the #55 rounds (see the retired chain in `docs/implementations/2026-09-02-*`) · #69 originally measured `352` at `fd9bb0d`; this review-fix measurement removes the 40 Issue #60 synthetic-trainer tests and moves the remaining Issue #57 tests to `tests/training/`, yielding 356 collected and passed · the 2026-09-08 review follow-ups add 4 more (3 in `tests/test_interfaces.py` for the section 9.2 immutability round trip, the isinstance narrowing rule and the package-docstring count pin, 1 in `tests/training/test_parameters.py` for section 9.1's unshared-membership case), so 356 + 4 = 360 · PR #80 adds 10 more (6 for the payload digest's `date` normalisation, 3 for the gated re-read at the CLI, 1 for `--payload-only` and `--compare-reread` agreeing), so 360 + 10 = 370 — but the value here is the direct measurement at the merge `8176d79`, which is the first commit whose tree contains both sides, not that sum · the branch's own pre-merge `290` at `0a4271b` described neither side of the merge and was superseded on PR #80 review (Rule 6) · `tests/test_file_snapshots.py` skips without `git` and `bash`, so a machine lacking either collects 370 but passes 361 with 9 skipped · direct measurement, never a branch-count derivation · resolved to one row per Rule 6 instead of keeping both, which is the defect `scripts/check_ids.py` rule N1 exists to catch · cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at |
...
| NC-025 | Distinct device states in the calibration archive | 504 | #45 §7, measured against `calibration-data` @ `f0930b9` (894 snapshot files, 43.6% byte-identical in their qubit block to the previous one) | 2026-08-30 | The unit NC-012 counts as of the 2026-08-30 decision · 504/234 ≈ 2.15 samples per trainable parameter, well below the 5× the floor is derived from (**corrected 2026-09-09, #56 FR-10**: this note read `504/126 ≈ 4.0` until the parameter count was measured rather than recalled — see NC-045; the 504 itself is unchanged) · the gate is **not** met · distinct is not independent: 504 is an upper bound on the independent-sample count (temporal correlation, see the NC-012 caveat), which only strengthens "not met" |
