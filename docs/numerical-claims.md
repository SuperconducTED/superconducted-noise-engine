...
| NC-044 | Wall-clock time for a full feature-distribution survey of the archive | 63.70 s | `python -m scripts.feature_distribution --repo . --ref 3d1569d18bcc007c35f3f628f79e678e6061bdc3 --out ...`, `runtime_seconds` from the printed summary, 975 files at `3d1569d` | 2026-09-08 | **Provisional, laptop** (Mert's) — `docs/verification/` holds only Burak-desktop records, and under architect decision C2 this PR does not get its own: the figure is superseded by the next batch record `docs/verification/2026-09-XX-phase-3-batch-N-burak-desktop.md` · one `git show` subprocess and one JSON parse per file, about 1.3 MB each, so roughly 1.2 GB of JSON on a warm object cache · `--limit N` exists so smoke runs do not pay it |
