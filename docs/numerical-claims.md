...
| NC-021 | Full test-suite size | 290 | `python -m pytest tests/ --collect-only -q -o addopts=""`, measured at `039afdc` | 2026-09-07 | Was `149` at `981f324`, then `166` at `1873625`, then `225` at `2af5c16`, `280` at `947fe3d`, and `287` at `04fa8d1` · review fixes add 3 tests: malformed-qubit rejection, incomplete-index backfill refusal, and poll-side undecidable-digest preservation/index omission · the complete suite passed at this reference: 290 passed in 32.87s · `tests/test_file_snapshots.py` skips without `git` and `bash`; see Rule 6 · cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at |
