...
| NC-032 | Calibration capture rate outside a known outage, 2026-08-12..14 | ≤ 70.2% (33 of ≥ 47) | Same enumeration as NC-031: 43 documents served inside the window plus 4 held-but-unserved = ≥ 47 proven to exist; `git ls-tree` on `calibration-data` holds 33 | 2026-09-02 | Upper bound, because the sweep itself undercounts (NC-031) · this window was **not** a flagged outage — it is ordinary operation, so the ≤ 19.3% of NC-029 is not the whole story and the shortfall is not confined to scheduler failures · the 14 documents in the `MISSED` rows are absent from every month directory on the branch and were recoverable at measurement time · 2026-09-07: the 14 `MISSED` rows were recovered by the #53 backfill, so all 47 control rows are now archived, and 3 further documents inside the same span that the 1 h sweep never served came with them (`20260812T081604000000Z`, `20260812T104835000000Z`, `20260812T183935000000Z`) — the window stands at **50 held of ≥ 50 proven** over `20260812T063125Z .. 20260814T174409Z`, measured by `git ls-tree` on `calibration-data` @ `1996bf6` · the `≥ 47` denominator was therefore itself an undercount, which is NC-031 arriving from an independent direction: the 1 h sweep served 43 of the 50 now known to exist, 86.0%, against NC-031's measured 87.9% · **value and `Last verified` are deliberately unchanged.** This row measures what the *poller* captured during ordinary operation as of 2026-09-02; restating it as ≤ 100% (50 of ≥ 50) would read as “capture is fine” when the window is complete only because #53 intervened on the very window it measures. A post-recovery capture rate is not bounded by this row — measuring one needs a window that has not been backfilled (#54) |
