...
| NC-047 | Current pipeline-health backfill | 537 distinct states from 936 documents; 42.6% duplication | At `calibration-data` @ `46f93c8`, run `PYTHONPATH=. python scripts/backfill_state_index.py --root <checkout>` then `PYTHONPATH=. python scripts/pipeline_health.py --root <checkout>` | 2026-09-05 | Candidate floors remain configuration, not a determination of training sufficiency; the commands operate only on the specified committed checkout. Renumbered from NC-042 before merge because PR #68 claims NC-042 for a different statistic and open PRs #87/#56 claim NC-045 and NC-046. |
