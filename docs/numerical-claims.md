...
| NC-021 | Full test-suite size | 186 | `python -m pytest tests/ --collect-only -q -o addopts=""` (tail line), run at `efa8c5c` on `mert/calibration-yield-issue-45` (the commit that adds them) | 2026-08-30 | Was `149` at `981f324`, then `166` at `1873625` · +20 on this branch: 2 for #46/#45 regressions, 16 for the probe's verdict table and sweep, 2 for the historical-response guard in `fetch_snapshot` · see Rule 6 · cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at |
