...
| NC-021 | Full test-suite size | 370 | `python -m pytest tests/ -q` and `python -m pytest tests/ --collect-only -q -o addopts=""`, measured on the merge of `645b4d1` into `feature/pipeline-health-dashboard` | 2026-09-08 | Supersedes 360 at `5397bb6`: this merge incorporates the Issue #57 training-target suite and the pipeline-health tests. Direct measurement, never a branch-count derivation; cite this row in verification runbooks rather than recalling a count, and state the commit it was measured at. |
